...
async function resolveInput(input) {
  ...
  throw new Error("Entrada no válida: se esperaba Buffer, URL string, o ruta de archivo");
}
...
async function imageToWebp(buffer) {
  try {
    ...
    throw new Error("Error al convertir imagen a webp: " + error.message);
  }
}
...
function videoToWebp(buffer) {
  return new Promise((resolve, reject) => {
    ...
    if (!buffer || buffer.length < 1000)
      return reject(new Error("Buffer de video no válido"));
    ...
    const timeout = setTimeout(() => {
      cleanup();
      reject(new Error("Tiempo de espera de conversión de video agotado"));
    }, 60000);
    ffmpeg(inputPath)
      .inputOptions(["-y", "-t", "6"])
      .outputOptions([
...
async function extendSocket(sock) {
  sock.sendImageAsSticker = async (jid, input, m, options = {}) => {
    ...
    try {
      ...
      throw new Error("Error al convertir imagen: " + err.message);
    }
    try {
  ...
  sock.forwardStickerPack = async (jid, packIdOrMessage, m) => {
    ...
    else throw new Error("Formato de mensaje de paquete de stickers no válido");
  ...
  sock.sendFile = async (jid, input, options = {}) => {
    ...
    if (Buffer.isBuffer(input)) {
      ...
      } else throw new Error("Entrada no válida");
    } else throw new Error("Tipo de entrada no válido");
    if (!mimetype) {
  ...
  sock.sendMedia = async function (
    ...
    if (Buffer.isBuffer(source)) {
      ...
      throw new Error(
        "La fuente debe ser un Buffer, URL string, ruta de archivo, objeto { url }, o payload de media",
      );
  ...
  sock.sendContact = async (jid, contacts, options = {}) => {
    ...
    const displayName =
      contactArray.length === 1
        ? contactArray[0].name || "Contacto"
        : `${contactArray.length} Contactos`;
    return sock.sendMessage(
      jid,

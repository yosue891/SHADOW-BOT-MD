...
> 🌐 *Link:* ${url}`
    ...
    if (isAudio) {
      const fixedAudio = await fixAudio(buffer)
      await conn.sendMessage(
        m.chat,
        { audio: fixedAudio, mimetype: 'audio/mpeg', fileName: `${fileName}.mp3` },
        { quoted: businessHeader }
      )
    } else {
...
async function fixAudio(buffer) {
  const input = `./in_audio_${Date.now()}.mp3`
  const output = `./out_audio_${Date.now()}.mp3`
  fs.writeFileSync(input, buffer)

  await new Promise((res, rej) => {
    spawn('ffmpeg', ['-y', '-i', input, '-b:a', '128k', output])

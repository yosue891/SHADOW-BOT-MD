...
let handler = async (m, { conn, participants, groupMetadata, text, args, usedPrefix }) => {
    ...
    let str;
    if (hayMencion) {
        str = `\`${name2}\` *acaba de bostezar frente a* \`${name}\`.`;
    } else if (via === 'quoted') {
        str = `\`${name2}\` *acaba de bostezar frente a* \`${name}\`.`;
    } else {
    ...
    if (m.isGroup) {
        try {
            ...
            await enviarReaccionAnime(conn, m, { reaccion: 'yawn', caption: str, mentions: [who] });
        } catch (e) {
            console.error('[anime-yawn] no se pudo enviar el GIF:', e.message)

...
let handler = async (m, { conn, participants, groupMetadata, text, args, usedPrefix }) => {
    ...
    if (m.isGroup) {
        try {
            ...
            await enviarReaccionAnime(conn, m, { reaccion: 'stare', caption: str, mentions: [who] });
        } catch (e) {
            console.error('[anime-look] no se pudo enviar el GIF:', e.message)

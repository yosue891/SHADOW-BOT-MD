...
export default {
  ...
  run: async (client, m, args, command, text, prefix) => {
    try {
      // Configuración del mensaje estilo Canal (Igual a la captura)
      const canalResponse = {
        contextInfo: {
          sidebarColor: "#000000",
          forwardingScore: 1,
          ...
          forwardedNewsletterMessageInfo: {
            newsletterJid: '120363203027814474@newsletter', // Jid de tu canal
            serverMessageId: 100,
            newsletterName: 'COLUMBINA CANAL' // Nombre que sale arriba en verde
          },
          externalAdReply: {
            title: "COLUMBINA MD",
            body: "666 MB • PNG",
            ...
            thumbnailUrl: "https://files.catbox.moe/zxjxhd.jpg", // Tu imagen de Columbina
            sourceUrl: "https://whatsapp.com/channel/0029VaJxgwCCt9fduVKYi61A" // Link de tu canal
          }
        }
      }
      ...
      if (args[0] === '-list') {
        ...
        return await client.sendMessage(m.chat, { text: helpText }, { quoted: m, ...canalResponse })
      }      
      ...
      const processImage = async (buffer, isWebpAnimated = false) => {
        if (isWebpAnimated) return buffer
      ...
      if (/image/.test(mime) || /webp/.test(mime)) {
        ...
        if ((quoted.msg || quoted).seconds > 20) return await client.sendMessage(m.chat, { text: '❖ El video no puede ser muy largo' }, { quoted: m, ...canalResponse })
        let buffer = await quoted.download()
        ...
        if (!url.match(/\.(jpe?g|png|gif|webp|mp4|mov|avi|mkv|webm)$/i)) {
          return await client.sendMessage(m.chat, { text: '❖ La URL debe ser de una imagen (jpg, png, gif, webp) o video (mp4, mov, avi, mkv, webm)' }, { quoted: m, ...canalResponse })
        }        
        ...
        return await client.sendMessage(m.chat, { text: `❀ Por favor, envía una imagen, video, sticker.\n> Usa *${prefix + command} -list* para ver formas y efectos` }, { quoted: m, ...canalResponse })
      }
    } catch (e) {
      return await client.sendMessage(m.chat, { text: 'Error: ' + e }, { quoted: m })
    }
  }
}

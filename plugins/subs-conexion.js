...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  let id = `${who.split`@`[0]}`
  let pathMichiJadiBot = path.join('./jadibot/', id)
...
export async function MichiJadiBot(options) {
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    async function connectionUpdate(update) {
      ...
      if (mcode && !sock.user && !sock.pairingDelivered && !sock.authState.creds.registered) {
        ...
        try {
          ...
          if (txtCode?.key) setTimeout(() => { sock.parentConn.sendMessage(sock.originalMsg.chat, { delete: txtCode.key }).catch(() => {}) }, 45000)
          if (codeBot?.key) setTimeout(() => { sock.parentConn.sendMessage(sock.originalMsg.chat, { delete: codeBot.key }).catch(() => {}) }, 45000)
        } catch (e) {
      ...
      if (qr && !mcode) {
        ...
        if (txtQR && txtQR.key) {
          setTimeout(() => { sock.parentConn.sendMessage(currentChat, { delete: txtQR.key }).catch(() => {}) }, 45000)
        }
        return
      } 

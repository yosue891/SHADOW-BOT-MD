...
if (!args[0]) return m.reply(`❀ Debes mencionar a quien quieras regalar *${global.currency}*.\n> Ejemplo » *${usedPrefix + command} 25000 @mencion*`)
...
if (!who) return m.reply(`ꕥ Debes mencionar a alguien para transferir *${global.currency}*.`)
...
if (user.bank < count) return m.reply(`ꕥ No tienes suficientes *${global.currency}* en el banco para transferir.`)
user.bank -= count
...
m.reply(`❀ Transferiste *¥${count.toLocaleString()} ${global.currency}* a *${name}*\n> Ahora tienes *¥${user.bank.toLocaleString()} ${global.currency}* en total en el banco.`, null, { mentions: [who] })
}

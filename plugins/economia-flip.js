...
if (!global.db.data.chats[m.chat].economy && m.isGroup) return conn.reply(m.chat, `《✦》Los comandos de *Economía* están desactivados en este grupo.\n\nUn *administrador* puede activarlos con el comando:\n» *${usedPrefix}economy on*`, m)
...
if (Math.abs(cantidad) < 100) return conn.reply(m.chat, `ꕥ La cantidad mínima para apostar es *100 ${global.currency}*.`, m)
...
if (cantidad > user.coin) return conn.reply(m.chat, `ꕥ No tienes suficientes *${global.currency}* para apostar, tienes *¥${user.coin.toLocaleString()} ${global.currency}*.`, m)
const resultado = Math.random() < 0.5 ? 'cara' : 'cruz'
const acierto = resultado === eleccion
const cambio = acierto ? cantidad : -cantidad
user.coin += cambio
if (user.coin < 0) user.coin = 0
const mensaje = `「✿」La moneda ha caído en *${capitalize(resultado)}* y has ${acierto ? 'ganado' : 'perdido'} *¥${Math.abs(cambio).toLocaleString()} ${global.currency}*!\n> Tu elección fue *${capitalize(eleccion)}*`

...
class Button extends BaseBuilder {
    ...
    async toCard() {
        ...
        if (this._data) {
            try {
                mediaHeader = await prepareWAMessageMedia(this._data, { upload: this.#client.waUploadToServer });
            } catch (e) {
                mediaHeader = this._data;
            }
        }
        return {
            body: {
    ...
    async build(jid, { ...options } = {}) {
        ...
        const builtMessage = {
            ...
            header: {
                ...
                ...(this._data
                    ? await prepareWAMessageMedia(this._data, { upload: this.#client.waUploadToServer }).catch(() => this._data)
                    : {}),
            },
            nativeFlowMessage: {
                messageParamsJson: JSON.stringify(this._params),
...
class Carousel extends BaseBuilder {
    ...
    async build(jid, { ...options } = {}) {
        if (this._cardBuilders && this._cardBuilders.length > 0) {
            for (const builder of this._cardBuilders) {
                const card = await builder._card.toCard();

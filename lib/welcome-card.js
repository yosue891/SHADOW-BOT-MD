...
async function getCanvas() {
  ...
  // 1. Intentar primero con el vendor precompilado que incluye librerías compartidas linux-x64
  try {
    ...
    // Continuar con canvas de npm si el vendor no coincide
  }

  // 2. Intentar con paquete npm 'canvas'
  try {
  ...
  const err = new Error("No se pudo cargar 'canvas' ni 'canvas-vendor'.")
  canvasError = err
  throw err
}

let fontsReady = false
function setupFonts(registerFont) {
  if (fontsReady || typeof registerFont !== 'function') return
...
export async function renderWelcomeCard({
  ...
  if (backgroundUrl) {
    try {
      bgImg = await loadImage(await fetchBuffer(backgroundUrl))
    } catch {}
  }
  if (!bgImg && fs.existsSync(LOCAL_BG)) {
    try { bgImg = await loadImage(fs.readFileSync(LOCAL_BG)) } catch {}
  }
  if (!bgImg && fs.existsSync(LOCAL_AVATAR)) {
    try { bgImg = await loadImage(fs.readFileSync(LOCAL_AVATAR)) } catch {}
  }

  let avatarImg = null
  if (avatarUrl) {
    try {
      avatarImg = await loadImage(await fetchBuffer(avatarUrl))
    } catch {}
  }

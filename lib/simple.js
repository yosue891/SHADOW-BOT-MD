        ...
        reply: {
            ...
            async value(jid, text = "", quoted, options) {
                ...
                const result = sendQueue.then(async () => {
                    ...
                    let replyVariant = 1;
                    try {
                        const s = global.db?.data?.settings?.[conn.user?.jid] || global.db?.data?.settings || {};
                    ...
                    try {
                        if (replyVariant === 1) {
                            ...
                            const fakeOrder = { key: { participant: "0@s.whatsapp.net", remoteJid: jid, fromMe: false }, message: { orderMessage: { orderId: "123456", itemCount: 999, status: 1, surface: 1, message: botname, orderTitle: "System Notification", sellerJid: "0@s.whatsapp.net", token: "ARU1+", totalAmount1000: "1000000", totalCurrencyCode: "IDR", thumbnail: getThumb() } } };
                            return await conn.sendMessage(jid, { text, ...options }, { quoted: fakeOrder, ...options });

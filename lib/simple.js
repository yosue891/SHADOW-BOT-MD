...
// Ourin (ourin-baileys) shares $protobuf.roots["default"] with @whiskeysockets/baileys and
// overwrites $root.proto on import, so proto.WebMessageInfo.fromObject starts returning
// instances of the new proto without our serialize getters. Use the current root proto.
function getCurrentProto() {
    try {
        const rootProto = $protobuf.roots?.["default"]?.proto
        if (rootProto?.WebMessageInfo) return rootProto
    } catch {}
    return proto
}
...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    // Mensaje original de Baileys (sin variantes). Las variantes lo usan por
    // dentro para no entrar en recursión con el wrapper de abajo.
    const _origSendMessage = conn.sendMessage.bind(conn);
    // Claves que indican que el mensaje NO es solo texto (menús con imagen,
    // ping, stickers, reacciones, borrados, ediciones, etc). Esos pasan directo.
    const _nonTextKeys = ["image", "video", "document", "audio", "sticker", "contacts", "contact", "location", "liveLocation", "poll", "pollCreation", "reaction", "delete", "forward", "protocolMessage", "viewOnceMessage", "interactiveMessage", "orderMessage", "requestPaymentMessage", "contactMessage", "groupInviteMessage", "listMessage", "buttonsMessage", "templateMessage", "edit"];
    function isPlainTextOnly(msg) {
        if (!msg || typeof msg !== "object" || typeof msg.text !== "string") return false;
        return !_nonTextKeys.some((k) => msg[k] !== undefined && msg[k] !== null);
    }
    // Núcleo de variantes setreply v1-v11 (v1 = ORIGINAL Shadow preservado).
    // Lo usan conn.reply (con cola anti-spam) y el wrapper de sendMessage (directo).
    async function dispatchVariant(jid, text, quoted, options) {
        if (Buffer.isBuffer(text)) {
            return await conn.sendFile(jid, text, "file", "", quoted, false, options);
        }
        let replyVariant = 1;
        try {
            const rawJid = conn.user?.jid || conn.user?.id || conn.user?.lid || '';
            const botJid = String(rawJid).replace(/:\d+@/, '@');
            const s = (botJid && global.db?.data?.settings?.[botJid]) || global.db?.data?.settings || {};
            replyVariant = Number(s?.replyVariant || global.db?.data?.settings?.replyVariant || 1);
            if (!replyVariant || replyVariant < 1 || replyVariant > 11) replyVariant = 1;
        } catch { replyVariant = 1; }
        const botname = global.botname || "Shadow-BOT-MD";
        const channelLink = global.channel || "https://whatsapp.com/channel/0029VbArz9fAO7RGy2915k3O";
        const newsletterJid = global.idcanal || global.channelRD?.id || "120363403739366547@newsletter";
        const newsletterName = global.channelRD?.name || botname;
        const getThumb = () => {
            try {
                if (fs.existsSync("./lib/catalogo.jpg")) return fs.readFileSync("./lib/catalogo.jpg");
                if (fs.existsSync("./imgs.jpg")) return fs.readFileSync("./imgs.jpg");
            } catch { }
            return null;
        };
        const getDocBuffer = () => {
            try {
                if (fs.existsSync("./package.json")) return fs.readFileSync("./package.json");
                if (fs.existsSync("./database.json")) return fs.readFileSync("./database.json");
            } catch { }
            return Buffer.from("Shadow-BOT-MD");
        };
        try {
            if (replyVariant === 1) {
                return await _origSendMessage(jid, { ...options, text }, { quoted, ...options });
            } else if (replyVariant === 2) {
                const fakeContact = {
                    key: { participant: "0@s.whatsapp.net", remoteJid: "status@broadcast" },
                    message: { contactMessage: { displayName: `Shadow ${botname}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:XL;ttname,;;;\nFN:ttname\nitem1.TEL;waid=13135550002:+1 (313) 555-0002\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, sendEphemeral: true } }
                };
                return await _origSendMessage(jid, { document: getDocBuffer(), mimetype: "image/png", fileName: botname, fileLength: 99999999999999, jpegThumbnail: getThumb(), caption: text, ...options }, { quoted: fakeContact, ...options });
            } else if (replyVariant === 3) {
                const thumb = getThumb();
                if (thumb) return await _origSendMessage(jid, { image: thumb, caption: text, ...options }, { quoted, ...options });
                return await _origSendMessage(jid, { ...options, text }, { quoted, ...options });
            } else if (replyVariant === 4) {
                return await _origSendMessage(jid, { text: `${channelLink}\n\n${text}`, contextInfo: { mentionedJid: options?.mentions || [], isForwarded: true, forwardingScore: 9, forwardedNewsletterMessageInfo: { newsletterJid, newsletterName, serverMessageId: Math.floor(Math.random() * 1000000) }, externalAdReply: { title: botname, body: botname, thumbnail: getThumb(), sourceUrl: channelLink, mediaType: 1, renderLargerThumbnail: true }, ...options?.contextInfo }, ...options }, { quoted, ...options });
            } else if (replyVariant === 5) {
                const orderMsg = generateWAMessageFromContent(jid, { orderMessage: { orderId: "FAKE-" + Date.now(), thumbnail: getThumb(), itemCount: 1, status: 1, surface: 1, message: text, orderTitle: "Pedido enviado por catálogo", sellerJid: null, token: null, totalAmount1000: "0", totalCurrencyCode: "GTQ", contextInfo: { mentionedJid: options?.mentions || [] } } }, { quoted, userJid: conn.user.id });
                return await conn.relayMessage(jid, orderMsg.message, { messageId: orderMsg.key.id });
            } else if (replyVariant === 6) {
                return await _origSendMessage(jid, { document: getDocBuffer(), mimetype: "image/png", fileName: botname, fileLength: 99999999999999, jpegThumbnail: getThumb(), caption: text, ...options }, { quoted, ...options });
            } else if (replyVariant === 7) {
                const msg = generateWAMessageFromContent(jid, { viewOnceMessage: { message: { messageContextInfo: {}, interactiveMessage: { header: { hasMediaAttachment: true, locationMessage: { degreesLatitude: 0, degreesLongitude: 0, name: botname, address: "Bot Wa Multi Device", jpegThumbnail: getThumb() } }, body: { text }, contextInfo: { mentionedJid: options?.mentions || [], isForwarded: true, forwardingScore: 9, ...options?.contextInfo }, nativeFlowMessage: { buttons: [] } } } } }, { quoted, userJid: conn.user.id });
                return await conn.relayMessage(jid, msg.message, { messageId: msg.key.id });
            } else if (replyVariant === 8) {
                return await conn.relayMessage(jid, { interactiveMessage: { header: { title: botname }, body: { text }, nativeFlowMessage: { buttons: [{ name: "inapp_signup", buttonParamsJson: "{}" }] }, contextInfo: { mentionedJid: options?.mentions || [], groupMentions: [], remoteJid: jid, forwardingScore: 999, isForwarded: true } } }, {});
            } else if (replyVariant === 9) {
                return await conn.relayMessage(jid, { orderMessage: { orderId: "WXX", thumbnail: getThumb(), itemCount: 1, status: "INQUIRY", surface: "CATALOG", message: text, orderTitle: "CONTOL", token: "whyuxD", totalAmount1000: "0", totalCurrencyCode: "IDR", messageVersion: 1, contextInfo: { mentionedJid: options?.mentions || [] } } }, {});
            } else if (replyVariant === 10) {
                return await conn.relayMessage(jid, { requestPaymentMessage: { currencyCodeIso4217: "IDR", amount1000: "75000000", requestFrom: quoted?.sender || jid, noteMessage: { extendedTextMessage: { text, contextInfo: { mentionedJid: options?.mentions || [] } } } } }, {});
            } else if (replyVariant === 11) {
                const delay = (ms) => new Promise(r => setTimeout(r, ms));
                const chars = [...botname];
                const sentMsg = await _origSendMessage(jid, { text: "..." }, { quoted });
                const key = sentMsg.key;
                ;(async () => { try { let iteration = 0; while (iteration < 20) { let cur = ""; for (const ch of chars) { cur += ch; await _origSendMessage(jid, { text: `${channelLink}\n\n${text}`, edit: key }); await delay(2000); iteration++; if (iteration >= 20) break; } await delay(1000); } } catch (e) { console.error("V11 Animation error:", e); } })();
                return sentMsg;
            }
        } catch (e) {
            console.error("Error en reply variant, usando original:", e?.message);
        }
        return await _origSendMessage(jid, { ...options, text }, { quoted, ...options });
    }
    // Todo texto enviado con sendMessage pasa por la variante activa.
    // Menús/ping (imagen), stickers, reacciones, borrados y ediciones pasan directo.
    conn.sendMessage = async function (jid, message = {}, opts = {}) {
        try {
            if (isPlainTextOnly(message)) {
                const { text, ...msgRest } = message;
                const merged = { ...msgRest, ...opts };
                delete merged.quoted;
                return await dispatchVariant(jid, text, opts?.quoted, merged);
            }
        } catch (e) {
            console.error("Error en sendMessage variant, usando original:", e?.message);
        }
        return await _origSendMessage(jid, message, opts);
    };
...
END:VCARD
        ...
        reply: {
            /**
             * Reply to a message with selectable variants (setreply v1-v11)
             * v1 = ORIGINAL Shadow (texto simple, preservado)
             * v2-v11 = portados de Akane-Assistant (mismo owner)
             * @param {String} jid
            ...
            async value(jid, text = "", quoted, options) {
                const sendQueue = global.__sendQueue || (global.__sendQueue = Promise.resolve());
                const result = sendQueue.then(() => dispatchVariant(jid, text, quoted, options));
                global.__sendQueue = result.then(() => new Promise(r => setTimeout(r, 600)));
                return result;
            },
        },
    ...
    dynamicButtons.push({
        ...
        sendButton: {
            async value(
                ...
                const interactiveMessage = {
                    ...
                    contextInfo: {
                        // Menciones dentro del mensaje interactivo (antes se
                        // perdían: options se pasaba solo a relayMessage)
        ...
        cMod: {
            ...
            value(jid, message, text = "", sender = conn.user.jid, options = {}) {
                ...
                return getCurrentProto().WebMessageInfo.fromObject(copy);
            },
            enumerable: true,
        },
        copyNForward: {
...
export function smsg(conn, m, hasParent) {
    ...
    const M = currentProto.WebMessageInfo || proto.WebMessageInfo
    // Ourin overwrites $root.proto after initial serialize, so new proto lacks getters
    if (M && !Object.getOwnPropertyDescriptor(M.prototype, 'text')?.get) {
        try { serialize() } catch (e) { console.error('re-serialize failed', e) }
    }
    try {
        m = M.fromObject(m);
        m.conn = conn;
...
export function serialize() {
    ...
    const _protoList = []
    if (!Object.getOwnPropertyDescriptor(proto.WebMessageInfo.prototype, 'text')?.get) _protoList.push(proto)
    try {
        const cur = getCurrentProto()
        if (cur && cur !== proto && cur?.WebMessageInfo?.prototype && !Object.getOwnPropertyDescriptor(cur.WebMessageInfo.prototype, 'text')?.get) _protoList.push(cur)
    } catch {}
    if (!_protoList.length) return
    for (const _P of _protoList) Object.defineProperties(_P.WebMessageInfo.prototype, {
        conn: {
            value: undefined,
            enumerable: false,
            writable: true,
        ...
        sender: {
            get() {
                try {
                    const raw = this.key?.fromMe
                        ? (this.conn?.user?.jid || this.conn?.user?.id)
                        : (this.key?.participantAlt || this.key?.remoteJidAlt || this.participantAlt || this.participant || this.key?.participant || this.chat || this.key?.remoteJid || "");
                    return safeDecodeJid(raw, this.conn);
                } catch (e) {
        ...
        fromMe: {
            get() {
                try {
                    const userId = this.conn?.user?.jid || this.conn?.user?.id || "";
                    const sender = this.sender || "";
                ...
                try {
                    ...
                    return Object.defineProperties(
                        JSON.parse(
                            ...
                            vM: {
                                get() {
                                    return getCurrentProto().WebMessageInfo.fromObject({
                                        key: {
                                            fromMe: this.fromMe,
                                            remoteJid: this.chat,
                                            id: this.id,
                            ...
                            copy: {
                                value() {
                                    const M = getCurrentProto().WebMessageInfo || proto.WebMessageInfo;
        ...
        mentionedJid: {
            get() {
                try {
                    // Array síncrono: src/index.js ya resuelve los @lid al recibir
                    // el mensaje, así que el crudo viene listo. Síncrono para que
                    // m.mentionedJid[0]/.length funcionen; `await` sobre un array
                    // sigue devolviendo el array, así que los que lo esperan no rompen.
        ...
        getQuotedObj: {
            value() {
                try {
                    ...
                    const q = getCurrentProto().WebMessageInfo.fromObject(

        ...
        reply: {
            ...
            async value(jid, text = "", quoted, options) {
                ...
                const result = sendQueue.then(async () => {
                    ...
                    try {
                        if (replyVariant === 1) {
                            return await conn.sendMessage(jid, { ...options, text }, { quoted, ...options });
                            ...
                            return await conn.sendMessage(jid, { document: getDocBuffer(), mimetype: "image/png", fileName: botname, fileLength: 99999999999999, jpegThumbnail: getThumb(), caption: text, ...options }, { quoted, ...options });
                            ...
                            const chars = [...botname];
                            const sentMsg = await conn.sendMessage(jid, { text: "..." }, { quoted });
                            const key = sentMsg.key;
                            ;(async () => { try { let iteration = 0; while (iteration < 20) { let cur = ""; for (const ch of chars) { cur += ch; await conn.sendMessage(jid, { text: `${channelLink}\n\n${text}`, edit: key }); await delay(2000); iteration++; if (iteration >= 20) break; } await delay(1000); } } catch (e) { console.error("V11 Animation error:", e); } })();
                            return sentMsg;
                        }
                    } catch (e) {
                        console.error("Error en reply variant, usando original:", e?.message);
                    }
                    const res = await conn.sendMessage(jid, { ...options, text }, { quoted, ...options });
                    return res;
                });

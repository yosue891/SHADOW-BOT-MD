...
function sticker6(img, url) {
    return new Promise(async (resolve, reject) => {
        ...
        const safeExt = getSafeExtension(type.ext);
        if (!safeExt) return reject(new Error("Unsupported file type"));
        const tmpName = crypto.randomBytes(16).toString("hex");
        const tmp = path.join(__dirname, `../tmp/${tmpName}.tmp`);
        const out = path.join(__dirname, `../tmp/${tmpName}.webp`);
        await fs.promises.writeFile(tmp, img);
        const Fffmpeg = /video/i.test(type.mime)
            ? fluent_ffmpeg(tmp).inputFormat(safeExt)

...
async function filesInit(pluginFolder = pluginFolder, pluginFilter = pluginFilter, conn) {
    ...
    // plugins/ está organizado por carpetas (owner/, ia/, menus/, ...), así que
    // la clave de cada plugin es su ruta relativa: "grupos/group-kick.js".
    const scanFiles = (dir = folder, base = '') => {
        const out = []
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
            if (entry.name.startsWith('.')) continue
            const rel = base ? `${base}/${entry.name}` : entry.name
            if (entry.isDirectory()) out.push(...scanFiles(join(dir, entry.name), rel))
            else if (pluginFilter(entry.name)) out.push(rel)
        }
        return out
    }

    await Promise.all(scanFiles().map(async filename => {
        try {

...
test('integración: el before recibe texto sin prefijo que el dispatcher descarta después', async t => {
  ...
  const m = mensajeRespuesta(g, g.p.actual.correcta.toLowerCase(), { quoted: undefined })
...
test('arranca con instrucciones y panel; conserva key y arranca reloj al publicarse', async t => {
  const g = await iniciar(t)
  assert.equal(g.sent.nuevos.length, 2)
  assert.match(g.sent.nuevos[0].text, /directamente en el grupo/)
...
test('cita ajena, chat ajeno y privado no puntúan', async t => {
  const g = await iniciar(t)
...
test('20 preguntas sin botones: solo dos mensajes base', async t => {
...
test('sin partida: letras se ignoran, comandos informan y ayuda explica el texto libre', async t => {
  reloj(t)
  const g = crearCtx()
  ...
  assert.match(ultimo(g), /sin prefijo/i)
  assert.match(ultimo(g), /Sin prefijo ni citar mensajes/)
})
...
async function libre(g, text, overrides = {}) {
  return respuestas.before({
    ...g.m, text, quoted: undefined,
    key: { remoteJid: g.m.chat, participant: P1, id: `texto-${++secuencia}` },
    react: async emoji => g.sent.reactions.push(emoji), ...overrides,
  }, g.ctx)
}

test('A/B/C/D escritos directamente, sin prefijo ni cita, se aceptan', async t => {
  reloj(t)
  for (const [i, letra] of ['a', 'B', ' c ', 'D'].entries()) {
    const g = crearCtx(`libre-${i}@g.us`)
    await handler(g.m, g.ctx)
    g.p = partidas.get(g.m.chat)
    g.p.actual.correcta = letra.trim().toUpperCase()
    await libre(g, letra)
    assert.equal(g.p.jugadores[P1].aciertos, 1)
    assert.deepEqual(g.sent.reactions, ['✅'])
  }
...
test('texto exacto de opción, sin cita y con normalización de tildes, puntúa', async t => {
  const g = await iniciar(t)
  g.p.actual = prepararPregunta({ cat: 'Prueba', q: '¿Quién?', o: ['Ángel Di María', 'Pelé', 'Messi', 'Zidane'], ok: 0 })
  await libre(g, '  ANGEL DI MARIA  ')
  assert.equal(g.p.jugadores[P1].aciertos, 1)
})
...
test('oportunidades independientes por jugador, también sin citas', async t => {
  ...
  const p = g.p.actual
  const mala = ['A', 'B', 'C', 'D'].find(l => l !== p.correcta)
  await libre(g, mala); await libre(g, mala)
  await libre(g, p.correcta, { sender: P2 })
  assert.equal(p.intentos.get(P1), 2)
...
test('reentrega de una respuesta libre no consume otra oportunidad', async t => {
  ...
  const p = g.p.actual
  const mala = ['A', 'B', 'C', 'D'].find(l => l !== p.correcta)
  const key = { id: 'MISMO-EVENTO', remoteJid: GRUPO, participant: P1 }
  await libre(g, mala, { key }); await libre(g, mala, { key })
  assert.equal(p.intentos.get(P1), 1)
  assert.equal(g.p.jugadores[P1].fallos, 1)
})

test('charla normal, IDs de botones viejos y comandos con prefijo no gastan intentos', async t => {
  const g = await iniciar(t)
  for (const texto of ['hola', 'a ver', 'buenas tardes', '.a', '.c', '.d', '!a', '/b', '#c', 'futbol:0123456789abcdef:0123456789abcdef:A', '']) {
    await libre(g, texto)
  }
  assert.equal(g.p.actual.intentos.size, 0)
  assert.deepEqual(g.p.jugadores, {})
...
test('partida completa responde sin citas: solo anuncio y panel editable, sin tarjetas', async t => {
  const g = await iniciar(t, 20)
  for (let i = 0; i < 20; i++) {
    await libre(g, g.p.actual.correcta)
    await avanzar(t, 2500)
...
test('parser acepta texto básico/efímero pero ignora controles interactivos anteriores', () => {
  for (const m of [
    { text: ' a ' }, { body: 'a' }, { message: { conversation: 'a' } },
    { message: { extendedTextMessage: { text: 'a' } } },
    { message: { ephemeralMessage: { message: { conversation: 'a' } } } },
    { msg: { text: 'a' } },
  ]) assert.equal(textoDeRespuesta(m), 'a')
  for (const m of [
    { text: 'A', mtype: 'interactiveResponseMessage' },
    { text: 'A', message: { interactiveResponseMessage: { nativeFlowResponseMessage: { paramsJson: '{}' } } } },
    { text: 'A', message: { buttonsResponseMessage: { selectedButtonId: 'A' } } },
    { text: 'A', msg: { nativeFlowResponseMessage: { paramsJson: '{}' } } },
    { text: 'A', message: { templateButtonReplyMessage: { selectedId: 'A' } } },
  ]) assert.equal(textoDeRespuesta(m), '')
})

test('botón antiguo con letra visible no se interpreta como respuesta libre', async t => {
  const g = await iniciar(t)
  await libre(g, g.p.actual.correcta, { message: {
    interactiveResponseMessage: { nativeFlowResponseMessage: { paramsJson: '{}' } },
  } })
  assert.equal(g.p.actual.intentos.size, 0)
})

test('sin partida no hay reacciones a letras sueltas ni texto de opciones', async t => {
  reloj(t)
  const g = crearCtx()
  await libre(g, 'a'); await libre(g, 'Lionel Messi')
  assert.equal(g.sent.nuevos.length, 0)
